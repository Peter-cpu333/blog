...
var handleModuleAssets = ({ config }) => {
  config.module.rule("images").test(/\.(png|jpe?g|gif|webp)(\?.*)?$/i).type("asset").generator({
    filename: "assets/img/[name].[contenthash:8][ext]"
  });
  config.module.rule("svg").test(/\.(svg)(\?.*)?$/i).type("asset/resource").generator({
    filename: "assets/img/[name].[contenthash:8][ext]"
  });
  config.module.rule("media").test(/\.(mp4|webm|ogg|mp3|wav|flac|aac)(\?.*)?$/i).type("asset/resource").generator({
...
// src/index.ts
var index_default = webpackBundler;
export {
